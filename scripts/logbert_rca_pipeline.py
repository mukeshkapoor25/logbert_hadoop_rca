...
def df_to_file(df, file_name, save_csv=False, csv_path=None):
    sequences = df["EventSequence"] if "EventSequence" in df else df
...
def detect_anomalies_and_explain():
    ...
    model_mistral = AutoModelForCausalLM.from_pretrained(MISTRAL_MODEL, torch_dtype=torch.float32).to(device)
    model_mistral.eval()
    ...
    train_scores = [compute_distance_to_center(model({k: v.to(device) for k, v in batch.items()})["cls_output"], center) for batch in tqdm(train_loader)]
